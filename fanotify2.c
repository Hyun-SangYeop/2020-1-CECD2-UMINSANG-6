  ...
  if (event->mask & FAN_OPEN_PERM)
    {
      ...
      printf ("Received event in path '%s'", (get_file_path_from_fd (event->fd, file_path, PATH_MAX) ? file_path : "unknown"));
      printf (" pid=%d (%s): \n",event->pid, (get_program_name_from_pid (event->pid, program_path, PATH_MAX) ? program_path : "unknown"));
      access.fd = event->fd;
