...
//changed
	...
	x=scanner(path);
	//if -1 is returned, detected file will be removed
	if(x==-1){
		char command[100];
		sprintf(command,"rm %s",path);
		printf("copy is blocked\n");
		system(command);
	}
	//otherwise copy process won't be blocked
	else{
		printf("succssfully copied\n");
	}
